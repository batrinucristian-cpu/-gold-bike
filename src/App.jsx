...
const fmt = (d) => d ? new Date(d).toLocaleDateString("ro-RO", { day: "2-digit", month: "long", year: "numeric" }) : "â€”";
const fmtShort = (d) => d ? new Date(d).toLocaleDateString("ro-RO", { day: "2-digit", month: "short", year: "numeric" }) : "â€”";

const STATUSES = ["Preluat", "ÃŽn lucru", "Gata", "Livrat"];
const SC = {
  "Preluat":  { bg: "#FFF8E1", text: "#F57F17", dot: "#FFC107" },
  "ÃŽn lucru": { bg: "#E3F2FD", text: "#0D47A1", dot: "#1E88E5" },
...
CONDIÈšII GENERALE DE SERVICE â€“ ${service.nume}

1. IDENTIFICARE PÄ‚RÈšI
Prestator: ${service.nume}, ${service.adresa}, CUI ${service.cui}, Tel: ${service.tel}
Client: ${cl?.nume || "â€”"}, ${cl?.adresa || "â€”"}, Tel: ${cl?.tel || "â€”"}

2. OBIECT CONTRACT
Clientul predÄƒ spre reparare bicicleta: ${o.bicicleta}
Defecte declarate la predare: ${o.defect}
Data predÄƒrii: ${fmt(o.data)}

3. DEVIZ ESTIMATIV
Clientul a luat la cunoÈ™tinÈ›Äƒ devizul estimativ È™i autorizeazÄƒ efectuarea lucrÄƒrilor. PreÈ›ul final poate varia faÈ›Äƒ de estimare dacÄƒ Ã®n cursul reparaÈ›iei se descoperÄƒ defecte suplimentare. Orice modificare semnificativÄƒ va fi comunicatÄƒ clientului Ã®nainte de execuÈ›ie.

4. TERMEN DE RIDICARE È˜I TAXÄ‚ DE DEPOZITARE
Clientul va fi notificat cÃ¢nd bicicleta este gata. DupÄƒ ${service.zileGratuite} zile lucrÄƒtoare de la notificare, se va percepe o taxÄƒ de depozitare de ${service.taxaZi} lei/zi calendaristicÄƒ. ${service.nume} nu rÄƒspunde pentru bunurile neridicate dupÄƒ 90 de zile.

5. LIMITAREA RÄ‚SPUNDERII
${service.nume} nu rÄƒspunde pentru defecte nedeclarate la predare, uzura naturalÄƒ sau daune independente de reparaÈ›ia efectuatÄƒ. RÄƒspunderea prestatorului este limitatÄƒ la valoarea lucrÄƒrilor efectuate.

6. PROTECÈšIA DATELOR (GDPR)
Datele personale ale clientului sunt prelucrate exclusiv Ã®n scopul prestÄƒrii serviciilor. Nu vor fi transmise terÈ›ilor fÄƒrÄƒ consimÈ›ÄƒmÃ¢nt. Clientul are dreptul de acces, rectificare È™i È™tergere. Contact: ${service.email}

Prin semnÄƒturÄƒ, clientul confirmÄƒ cÄƒ a citit È™i acceptat toate condiÈ›iile de mai sus.
`.trim();
...
function getDevizHtml(o, cl, service) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Deviz â€“ ${cl?.nume}</title>
  ...
  <div class="header"><div><h1>ðŸ”§ ${service.nume}</h1><div style="color:#666;margin-top:4px">${service.adresa} Â· ${service.tel} Â· ${service.email}</div><div style="color:#888;font-size:11px">CUI: ${service.cui}</div></div><div style="text-align:right"><div class="badge">DEVIZ DE LUCRÄ‚RI</div><div style="margin-top:6px;font-size:12px">Nr. ${o.id.toUpperCase()}<br>Data: ${fmt(o.data)}</div></div></div>
  <h2>Date client & bicicletÄƒ</h2>
  <table><tr><th>Client</th><td>${cl?.nume||"â€”"}</td><th>Telefon</th><td>${cl?.tel||"â€”"}</td></tr><tr><th>AdresÄƒ</th><td>${cl?.adresa||"â€”"}</td><th>Email</th><td>${cl?.email||"â€”"}</td></tr><tr><th>BicicletÄƒ</th><td colspan="3">${o.bicicleta}</td></tr><tr><th>Defecte raportate</th><td colspan="3">${o.defect}</td></tr>${o.lucrarilEfectuate?`<tr><th>LucrÄƒri efectuate</th><td colspan="3">${o.lucrarilEfectuate}</td></tr>`:""}</table>
  <h2>Piese Ã®nlocuite</h2>
  <table><tr><th>Denumire</th><th>Cant.</th><th>PreÈ› unitar</th><th>Total</th></tr>${(o.piese||[]).map(p=>`<tr><td>${p.nume}</td><td style="text-align:center">${p.cant}</td><td style="text-align:right">${p.pret} lei</td><td style="text-align:right">${Number(p.cant)*Number(p.pret)} lei</td></tr>`).join("")}<tr><td colspan="3" style="text-align:right;font-weight:bold">Subtotal piese</td><td style="text-align:right">${totalP} lei</td></tr><tr><td colspan="3" style="text-align:right;font-weight:bold">ManoperÄƒ</td><td style="text-align:right">${o.manopera||0} lei</td></tr></table>
  <div class="total">TOTAL DE PLATÄ‚: ${total} lei</div>
  <div class="legal">${LEGAL(o,cl,service)}</div>
  <div class="sig"><div class="sig-box"><div class="sig-line">Prestator: ${service.nume}</div></div><div class="sig-box">${o.semnatura ? `<img src="${o.semnatura}" />` : ""}<div class="sig-line">Client: ${cl?.nume||"â€”"}<br>Data: ${fmt(today())}</div></div></div>
  </body></html>`;
}
  ...
  return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={onClose}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#F2F3F8",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:wide?560:480,maxHeight:"92vh",overflowY:"auto",padding:"24px 20px 40px",animation:"slideUp 0.25s ease"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <button onClick={onClose} style={{background:"#EBEBF0",border:"none",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontSize:15}}>âœ•</button>
      </div>
      {children}
    </div>
  </div>;
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxWidth:500,maxHeight:"90vh",overflowY:"auto",padding:"20px 20px 32px"}}>
        <div style={{fontWeight:900,fontSize:16,marginBottom:10}}>âœï¸ SemnÄƒturÄƒ & acordul clientului</div>
        ...
        <div style={{fontSize:12,color:"#E63946",fontWeight:700,marginBottom:8}}>Prin semnÄƒturÄƒ confirm cÄƒ am citit È™i accept condiÈ›iile de mai sus:</div>
        ...
        <div style={{display:"flex",gap:8,marginTop:12}}>
          <Btn onClick={clear} color="#888" outline small>È˜terge</Btn>
          <Btn onClick={onClose} color="#888" outline small>AnuleazÄƒ</Btn>
          <Btn onClick={()=>onSave(canvasRef.current.toDataURL("image/png"))} small>SalveazÄƒ</Btn>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  // â”€â”€ Firebase listeners â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
  ...
  // â”€â”€ New order form â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Photos â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Piese â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Comenzi piese â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if(loading) return <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#F2F3F8",flexDirection:"column",gap:16}}><div style={{fontSize:40}}>ðŸ”§</div><div style={{fontWeight:700,color:"#1a1a2e"}}>Gold Bike Service</div><div style={{color:"#aaa",fontSize:14}}>Se Ã®ncarcÄƒ datele...</div></div>;
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#F2F3F8",minHeight:"100vh",maxWidth:480,margin:"0 auto",paddingBottom:80}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1a1a2e 0%,#16213e 100%)",padding:"48px 20px 24px",color:"#fff"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontSize:22,fontWeight:900,letterSpacing:-0.5}}>ðŸš² Gold Bike <span style={{color:"#E63946"}}>Service</span></div>
            <div style={{fontSize:11,opacity:0.55,marginTop:2}}>Management complet service biciclete</div>
          </div>
          <button onClick={()=>setShowProg(true)} style={{background:"rgba(230,57,70,0.2)",border:"1.5px solid rgba(230,57,70,0.5)",borderRadius:10,padding:"7px 12px",color:"#E63946",fontSize:11,fontWeight:700,cursor:"pointer"}}>ðŸ“… Revizii</button>
        </div>
        <div style={{display:"flex",gap:8,marginTop:16}}>
          {[["Preluate","Preluat"],["ÃŽn lucru","ÃŽn lucru"],["Gata","Gata"],["Livrate","Livrat"]].map(([lb,key])=>(
      ...
      <div style={{display:"flex",background:"#fff",borderBottom:"1px solid #EBEBEB"}}>
        {[["comenzi","ðŸ“‹ Comenzi"],["clienti","ðŸ‘¤ ClienÈ›i"]].map(([k,l])=>(
      ...
      <div style={{padding:"14px 14px 0"}}>
        {tab==="comenzi"&&(<>
          <div style={{position:"relative",marginBottom:10}}>
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="CautÄƒ client, bicicletÄƒ, defect..." style={{width:"100%",padding:"10px 12px 10px 34px",borderRadius:12,border:"1.5px solid #E0E0E0",fontSize:13,outline:"none",background:"#fff",boxSizing:"border-box"}}/>
          </div>
          ...
          {filtered.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:40,fontSize:14}}>Nicio comandÄƒ. ApasÄƒ + pentru a adÄƒuga!</div>}
          {filtered.map(o=>{
            const cl=clients.find(c=>c.id===o.clientId);
            ...
            return (
              <div key={o.id} onClick={()=>setSel(o.id)} style={{background:"#fff",borderRadius:14,padding:14,marginBottom:10,boxShadow:"0 1px 6px rgba(0,0,0,0.07)",cursor:"pointer",borderLeft:`4px solid ${SC[o.status]?.dot||"#ccc"}`,transition:"transform 0.12s"}}
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:5}}>
                  <div><div style={{fontWeight:700,fontSize:14,color:"#1a1a2e"}}>{cl?.nume||"â€”"}</div><div style={{fontSize:11,color:"#aaa",marginTop:1}}>{o.bicicleta}</div></div>
                  <Badge s={o.status}/>
                </div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#bbb",flexWrap:"wrap",gap:4}}>
                  <span>ðŸ“ž {cl?.tel||"â€”"}</span>
                  <span style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {(o.poze||[]).length>0&&<span>ðŸ“·{o.poze.length}</span>}
                    {o.semnatura&&<span>âœï¸</span>}
                    {pending>0&&<span style={{color:"#FF7043",fontWeight:700}}>ðŸ“¦{pending}</span>}
                    {totalCost(o)>0&&<span style={{color:"#43A047",fontWeight:700}}>{totalCost(o)} lei</span>}
                    <span>ðŸ“…{fmtShort(o.data)}</span>
                  </span>
                </div>
              </div>
            );
        ...
        {tab==="clienti"&&clients.map(c=>{
          ...
          return <Card key={c.id}>
            ...
            <div style={{fontSize:12,color:"#888",marginTop:2}}>ðŸ“ž {c.tel} {c.email&&`Â· ${c.email}`}</div>
            {c.adresa&&<div style={{fontSize:11,color:"#bbb",marginTop:1}}>ðŸ“ {c.adresa}</div>}
            <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6}}>{[...new Set(cos.map(o=>o.bicicleta))].map((b,i)=><span key={i} style={{background:"#F0F4FF",borderRadius:8,padding:"3px 10px",fontSize:11,color:"#3949AB"}}>ðŸš² {b}</span>)}</div>
            <div style={{marginTop:6,fontSize:11,color:"#bbb"}}>{cos.length} comenzi Â· Total: {cos.reduce((s,o)=>s+totalCost(o),0)} lei</div>
          </Card>;
        })}
      </div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"#fff",borderTop:"1px solid #EBEBEB",display:"flex"}}>
        {[["comenzi","ðŸ“‹","Comenzi"],["clienti","ðŸ‘¤","ClienÈ›i"]].map(([k,ic,lb])=>(
      ...
      {sel&&selOrder&&(()=>{
        ...
        return <Modal title="Detalii comandÄƒ" onClose={()=>setSel(null)} wide>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <Btn onClick={()=>printDeviz(selOrder,cl,SERVICE)} color="#1565C0" outline small style={{flex:1}}>ðŸ–¨ï¸ Print</Btn>
            <Btn onClick={()=>downloadDeviz(selOrder,cl,SERVICE)} color="#1565C0" small style={{flex:1}}>ðŸ“¥ DescarcÄƒ deviz</Btn>
          </div>
          <Card><Label t="Client"/><div style={{fontWeight:800,fontSize:15}}>{cl?.nume}</div><div style={{fontSize:13,color:"#888",marginTop:2}}>ðŸ“ž {cl?.tel} {cl?.email&&`Â· ${cl.email}`}</div>{cl?.adresa&&<div style={{fontSize:11,color:"#bbb",marginTop:1}}>ðŸ“ {cl.adresa}</div>}</Card>
          <Card><Label t="BicicletÄƒ & defecte"/><div style={{fontWeight:700,fontSize:14,marginBottom:4}}>ðŸš² {selOrder.bicicleta}</div><div style={{fontSize:13,color:"#555"}}>{selOrder.defect}</div></Card>
          <Card><Label t="ObservaÈ›ii iniÈ›iale (client)"/><TA value={selOrder.observatii||""} onChange={v=>patchOrder(sel,{observatii:v})} placeholder="Ce a spus clientul..."/></Card>
          <Card style={{borderLeft:"3px solid #43A047"}}><Label t="ðŸ”§ LucrÄƒri efectuate (mecanic)"/><TA value={selOrder.lucrarilEfectuate||""} onChange={v=>patchOrder(sel,{lucrarilEfectuate:v})} placeholder="Descrie exact ce s-a reparat..." rows={4}/></Card>
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><Label t="Piese Ã®nlocuite"/><Btn onClick={()=>addPiesa(sel)} small color="#3949AB">+ PiesÄƒ</Btn></div>
            {(selOrder.piese||[]).length===0&&<div style={{fontSize:12,color:"#bbb"}}>Nicio piesÄƒ</div>}
            {(selOrder.piese||[]).map(p=>(
              <div key={p.id} style={{display:"flex",gap:5,marginBottom:7,alignItems:"center"}}>
                ...
                <button onClick={()=>delPiesa(sel,p.id)} style={{background:"#FFEBEE",border:"none",borderRadius:8,padding:"7px 9px",cursor:"pointer",color:"#E63946",fontWeight:700}}>âœ•</button>
              </div>
            ))}
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10,paddingTop:10,borderTop:"1px dashed #E0E0E0"}}>
              <span style={{fontSize:13,fontWeight:700,color:"#555"}}>ManoperÄƒ (lei):</span>
          ...
          <Card style={{borderLeft:"3px solid #FF7043"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><Label t="ðŸ“¦ Piese de comandat"/><Btn onClick={()=>addComanda(sel)} small color="#FF7043">+ AdaugÄƒ</Btn></div>
            {(selOrder.comenziPiese||[]).length===0&&<div style={{fontSize:12,color:"#bbb"}}>Nicio piesÄƒ de comandat</div>}
            {(selOrder.comenziPiese||[]).map(c=>(
              <div key={c.id} style={{marginBottom:8,padding:"10px",background:"#FFF8F6",borderRadius:10,border:"1px solid #FFE0D0"}}>
                <div style={{display:"flex",gap:5,marginBottom:6}}>
                  <input value={c.nume} onChange={e=>patchComanda(sel,c.id,{nume:e.target.value})} placeholder="Denumire piesÄƒ" style={{flex:2,padding:"6px 9px",borderRadius:7,border:"1.5px solid #E0E0E0",fontSize:12,outline:"none"}}/>
                  ...
                  <button onClick={()=>delComanda(sel,c.id)} style={{background:"#FFEBEE",border:"none",borderRadius:7,padding:"6px 9px",cursor:"pointer",color:"#E63946",fontWeight:700}}>âœ•</button>
                </div>
          ...
          <Card style={{borderLeft:"3px solid #1565C0"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><Label t="ðŸ“· Fotografii bicicletÄƒ"/><Btn onClick={()=>fileRef.current?.click()} small color="#1565C0">+ AdaugÄƒ</Btn><input ref={fileRef} type="file" accept="image/*" multiple capture="environment" style={{display:"none"}} onChange={handlePhotos}/></div>
            {(selOrder.poze||[]).length===0
              ?<div style={{background:"#F0F4FF",borderRadius:10,padding:"20px",textAlign:"center",color:"#90A4AE",fontSize:13,border:"2px dashed #BBDEFB"}}><div style={{fontSize:28,marginBottom:4}}>ðŸ“·</div>ApasÄƒ + pentru a fotografia bicicleta</div>
              :<><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{(selOrder.poze||[]).map(pz=><div key={pz.id} style={{position:"relative"}}><img src={pz.url} alt={pz.name} onClick={()=>setPreviewPoza(pz.url)} style={{width:90,height:90,objectFit:"cover",borderRadius:12,border:"2px solid #BBDEFB",display:"block",cursor:"pointer"}}/><button onClick={async()=>{const o=orders.find(x=>x.id===sel);await patchOrder(sel,{poze:o.poze.filter(x=>x.id!==pz.id)});}} style={{position:"absolute",top:-7,right:-7,background:"#E63946",border:"2px solid #fff",borderRadius:"50%",width:22,height:22,color:"#fff",fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900}}>âœ•</button></div>)}</div><div style={{marginTop:8,fontSize:11,color:"#90A4AE"}}>{(selOrder.poze||[]).length} fotografii</div></>
            }
          </Card>
          <Card>
            <Label t="Data notificÄƒrii client"/>
            ...
            {selOrder.dataNotificare&&(()=>{
              ...
              if(days>0) return <div style={{marginTop:8,background:"#FFEBEE",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#E63946",fontWeight:700}}>âš ï¸ TaxÄƒ depozitare: {days} zile Ã— {SERVICE.taxaZi} lei = {days*SERVICE.taxaZi} lei</div>;
              const ramas=SERVICE.zileGratuite-Math.ceil((new Date()-new Date(selOrder.dataNotificare))/(1000*60*60*24));
              return <div style={{marginTop:8,background:"#E8F5E9",borderRadius:8,padding:"8px 12px",fontSize:12,color:"#388E3C"}}>âœ… ÃŽn termen â€“ mai sunt {ramas} zile gratuite</div>;
            })()}
          </Card>
          <Card style={{borderLeft:"3px solid #E63946"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><Label t="SemnÄƒturÄƒ & acord client"/><Btn onClick={()=>setShowSig(true)} small color={selOrder.semnatura?"#888":"#E63946"} outline={!!selOrder.semnatura}>{selOrder.semnatura?"âœï¸ ResemneazÄƒ":"âœï¸ SemneazÄƒ"}</Btn></div>
            {selOrder.semnatura?<img src={selOrder.semnatura} alt="SemnÄƒturÄƒ" style={{maxWidth:"100%",border:"1.5px solid #E0E0E0",borderRadius:10,background:"#FAFAFA"}}/>:<div style={{fontSize:12,color:"#bbb",padding:"16px 0",textAlign:"center"}}>Nesemnat</div>}
          </Card>
          <Card><Label t="UrmÄƒtoarea revizie"/><input type="date" value={selOrder.nextRevizieData||""} onChange={e=>patchOrder(sel,{nextRevizieData:e.target.value})} style={{width:"100%",padding:"10px 12px",borderRadius:10,border:"1.5px solid #E0E0E0",fontSize:14,outline:"none",boxSizing:"border-box",background:"#FAFAFA"}}/></Card>
          {bikeHistory.length>1&&<Card><Label t={`Istoric ${selOrder.bicicleta}`}/>{bikeHistory.map(h=><div key={h.id} onClick={()=>setSel(h.id)} style={{padding:"10px 0",borderBottom:"1px solid #F0F0F0",cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,fontWeight:600,color:"#1a1a2e"}}>{fmt(h.data)}</span><Badge s={h.status}/></div><div style={{fontSize:12,color:"#777",marginTop:3}}>{h.defect}</div>{h.lucrarilEfectuate&&<div style={{fontSize:11,color:"#43A047",marginTop:2}}>âœ” {h.lucrarilEfectuate.slice(0,60)}{h.lucrarilEfectuate.length>60?"â€¦":""}</div>}{totalCost(h)>0&&<div style={{fontSize:11,color:"#888",marginTop:1}}>Cost: {totalCost(h)} lei</div>}</div>)}</Card>}
        </Modal>;
      })()}

      {/* NEW ORDER */}
      {showNew&&<Modal title="ComandÄƒ nouÄƒ" onClose={()=>setShowNew(false)}>
        <Card><Label t="Client existent"/>
          <select value={form.clientId} onChange={e=>setF("clientId")(e.target.value)} style={{width:"100%",padding:"10px 12px",borderRadius:10,border:"1.5px solid #E0E0E0",fontSize:14,outline:"none",background:"#FAFAFA"}}>
            <option value="">â€” SelecteazÄƒ client existent â€”</option>
            {clients.map(c=><option key={c.id} value={c.id}>{c.nume} ({c.tel})</option>)}
          </select>
        </Card>
        {!form.clientId&&<Card><Label t="Sau client nou"/>{[["numeNou","Nume complet"],["telNou","Telefon"],["emailNou","Email"],["adresaNou","AdresÄƒ"]].map(([k,ph])=><div key={k} style={{marginBottom:8}}><input value={form[k]} onChange={e=>setF(k)(e.target.value)} placeholder={ph} style={{width:"100%",padding:"9px 12px",borderRadius:10,border:"1.5px solid #E0E0E0",fontSize:13,outline:"none",boxSizing:"border-box",background:"#FAFAFA"}}/></div>)}</Card>}
        <Card><Label t="BicicletÄƒ"/><input value={form.bicicleta} onChange={e=>setF("bicicleta")(e.target.value)} placeholder="ex. Trek FX3 â€“ Negru" style={{width:"100%",padding:"9px 12px",borderRadius:10,border:"1.5px solid #E0E0E0",fontSize:13,outline:"none",boxSizing:"border-box",background:"#FAFAFA"}}/></Card>
        <Card><Label t="Defecte raportate"/><TA value={form.defect} onChange={setF("defect")} placeholder="Descrie defectele..."/></Card>
        <Card><Label t="ObservaÈ›ii iniÈ›iale"/><TA value={form.observatii} onChange={setF("observatii")} placeholder="OpÈ›ional" rows={2}/></Card>
        <Card><Label t="Revizie planificatÄƒ"/><input type="date" value={form.nextRevizieData} onChange={e=>setF("nextRevizieData")(e.target.value)} style={{width:"100%",padding:"9px 12px",borderRadius:10,border:"1.5px solid #E0E0E0",fontSize:14,outline:"none",boxSizing:"border-box",background:"#FAFAFA"}}/></Card>
        <div style={{display:"flex",gap:10}}><Btn onClick={()=>setShowNew(false)} color="#888" outline>AnuleazÄƒ</Btn><Btn onClick={handleSaveOrder} disabled={!form.bicicleta||!form.defect||(!form.clientId&&!form.numeNou)}>SalveazÄƒ</Btn></div>
      </Modal>}
      ...
      {previewPoza&&<div onClick={()=>setPreviewPoza(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
        ...
        <button onClick={()=>setPreviewPoza(null)} style={{position:"absolute",top:20,right:20,background:"rgba(255,255,255,0.2)",border:"none",borderRadius:"50%",width:40,height:40,color:"#fff",fontSize:20,cursor:"pointer"}}>âœ•</button>
      </div>}
      ...
      {showProg&&<Modal title="ðŸ“… ProgramÄƒri revizii" onClose={()=>setShowProg(false)}>
        {revizii.length===0&&<div style={{textAlign:"center",color:"#bbb",padding:30}}>Nicio revizie planificatÄƒ</div>}
        {revizii.map(o=>{
          const cl=clients.find(c=>c.id===o.clientId);
          ...
          return <div key={o.id} onClick={()=>{setShowProg(false);setSel(o.id);}} style={{background:past?"#FFEBEE":urgent?"#FFF8E1":"#fff",borderRadius:12,padding:14,marginBottom:10,cursor:"pointer",border:`1.5px solid ${past?"#FFCDD2":urgent?"#FFE082":"#E0E0E0"}`}}>
            <div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontWeight:700,fontSize:14}}>{cl?.nume}</div><span style={{fontSize:11,fontWeight:700,color:past?"#E63946":urgent?"#F57F17":"#43A047"}}>{past?`ÃŽntÃ¢rziat ${-days}z`:days===0?"Azi!":days===1?"MÃ¢ine":`Ã®n ${days} zile`}</span></div>
            <div style={{fontSize:12,color:"#888",marginTop:2}}>ðŸš² {o.bicicleta}</div>
            <div style={{fontSize:11,color:"#bbb",marginTop:4}}>ðŸ“… {fmt(o.nextRevizieData)}</div>
          </div>;
        })}
      </Modal>}
    </div>
